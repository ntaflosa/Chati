...
Vorgefertigte Vorlagen für die häufigsten Anwendungsfälle – ein Klick befüllt alle Felder automatisch mit optimalen Standardwerten. Der Katalog ist vollständig zweisprachig (Deutsch / Englisch) und wechselt automatisch mit der UI-Sprache.
...
Alle Parameter sind frei kombinierbar – wahlweise als freies Formular oder im **geführten Wizard-Modus** (Bereiche öffnen sich schrittweise mit „Weiter →"-Schaltflächen, vorheriger Bereich schließt automatisch):
...
### Prompt-Bibliothek
Die Bibliothek bietet zwei Bereiche:
- **Meine Prompts** – eigene Prompts benennen, lokal speichern, laden, kopieren und löschen (localStorage)
- **Beispiele** – kuratierte Community-Prompts für alle 9 Kategorien, sofort verwendbar und filterbar

### Community-Beispiel-Prompts
Über 20 fertige Prompts zu allen Kategorien, um Nutzer zu inspirieren. Neue Prompts lassen sich durch einfaches Bearbeiten von `examples.js` hinzufügen – kein Backend, kein Build-Schritt nötig.
...
### Sprache (DE / EN)
Die gesamte App-Oberfläche, der Prompt-Katalog sowie die Prompt-Ausgabe sind wahlweise auf Deutsch oder Englisch umschaltbar.

### Teilen & Exportieren
- Prompt per Knopfdruck in die Zwischenablage kopieren
- Als `.txt`-Datei exportieren
- **Share-Link generieren** – URL enthält alle Formularfelder und die aktive Sprache (`?lang=en`)
...
├── catalog.js      # Prompt-Katalog: DE + EN Vorlagen als JS-Array (leicht erweiterbar)
├── examples.js     # Community-Beispiel-Prompts: einfach bearbeiten, kein Backend nötig
...
In `catalog.js` ein weiteres Objekt ans `PROMPT_CATALOG`-Array anhängen (und optional ans `PROMPT_CATALOG_EN`-Array für die englische Version):
...
```js
{
  ...
  category: 'Content',   // muss einer der 9 Kategorien entsprechen
  lang: 'de',            // 'de' oder 'en'
  text: 'Erstelle einen …',
},
```

Datei speichern, hochladen – fertig. Kein Build, kein Backend.
...
### v1.5
- **Zweisprachiger Katalog (DE/EN)**: Vollständiger englischer Prompt-Katalog mit 38 Vorlagen; wechselt automatisch mit der UI-Sprache
- **Community-Beispiel-Prompts**: Neue „Beispiele"-Registerkarte in der Bibliothek mit 20+ kuratierten Prompts für alle 9 Kategorien
- **Verbesserter Share-Link**: URL enthält jetzt auch die aktive Sprache (`?lang=en`)
- **Wizard-Verbesserung**: Beim Klick auf „Weiter →" wird der aktuelle Bereich automatisch geschlossen, bevor der nächste öffnet
